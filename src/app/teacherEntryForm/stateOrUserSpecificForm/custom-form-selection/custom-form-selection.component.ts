...
export class CustomFormSelectionComponent implements OnInit {
  ...
  optionalChechBox(value){
    ...
    console.log(this.productForm.get('quantities'))
    
    // alert(this.otherSelected)
  }
  ...
  onselectedlevel(event){
    ...
    if(this.selectedLevel == 'Multiple Choice'){
      this.setLableType = 'multiChoice'
    }else if(this.selectedLevel == 'Checkbox'){
      this.setLableType = 'checkbox'
    }else if(this.selectedLevel == 'Dropdown'){
      this.setLableType = 'dropdown'
    }else if(this.selectedLevel == 'Single Text'){
      this.setLableType = 'singleText'
    ...
    arr.controls = [];
    if(this.setLableType == 'multiChoice' || this.setLableType=='checkbox' || this.setLableType=='dropdown'){
      this.onValid=true;
    }else{
      this.onValid=false;
    }

    if(this.setLableType != 'singleText'){
      if(this.setLableType == 'dateTime'){
        var j=1;
      }else{
        var j=4;
      }
